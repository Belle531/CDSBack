...
/**
 * Register a new user - matches RegisterView.jsx form structure
 */
const register = async (event) => {
  try {
    ...
    // Add preferredLanguage from selectedLanguage if provided
    if (body.selectedLanguage && !body.preferredLanguage) {
      body.preferredLanguage = body.selectedLanguage;
    }
    ...
    // Return user data (without password hash) - matches frontend expectation
    ...
    return response(201, {
      ...
      user: {
        ...userResponse,
        uid: userId // Add uid field that frontend expects
      },
      token
    });
...
const login = async (event) => {
  try {
    ...
    return response(200, {
      ...
      user: {
        ...userResponse,
        uid: user.userId // Add uid field that frontend expects
      },
      token
    });
